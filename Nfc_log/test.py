...
SKIPPED = []
argp = ArgumentParser()
argp.add_argument("-c", "--couchdb", help="http://admin:admin@127.0.0.1:5984", required=True)
argp.add_argument("-m", "--mongodb", help="mongodb://localhost:27017", required=True)
    ...
    if db not in ('admin', 'local'):  # omitting  special internal db's, 'admin'(DBS[0]) and 'local'(DBS[2])
        ...
        for col in cols:
            ...
            for x in CLIENT[db][col].find():  # iterate though all records inside a collection
                ...
                try:
                    response = requests.Session().get(URL+'/'+db+'-'+col, auth=('admin', 'admin'), headers=HEADERS, verify=False)
                    if 'error' in response.json():  # db doesnt exist on destination, create it
                        print('Creating a new database {} in CouchDB'.format(db+'-'+col))
                        output = requests.Session().put(URL+'/'+db+'-'+col, auth=('admin', 'admin'), headers=HEADERS, verify=False)
                        if output.status_code == 201 or 202:
                            DB_COUNT += 1
                    # insert documents in couchDB database.
                    output = requests.Session().post(URL+'/'+db+'-'+col, auth=('admin', 'admin'), headers=HEADERS,
...
# python test.py -c 'http://admin:admin@127.0.0.1:5984' -m 'mongodb://localhost:27017'
